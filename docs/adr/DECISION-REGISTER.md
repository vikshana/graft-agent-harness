...
| **D33** | **Back-channel (cancel/signal/approve/request-control/release-control) is plain REST**, idempotency-keyed to the run, identical across web, Grafana, and Slack — **locks R7** (superseding its "recommendation" status). Non-driver callers are rejected per D32's soft-lock, or redirected to `request-control`. **Signal delivery is a Postgres signal table + `LISTEN/NOTIFY`**, reusing D30's substrate — explicitly **provisional pending `04-durable-execution.md`**; revisit if Temporal (or another durable-execution engine with native signal delivery) is chosen as orchestrator, since it could replace or wrap this mechanism. **Cancel/signal is checked at every tool-call boundary** (not only at coarse checkpoints) — a concrete requirement carried into the durable-execution design: the execution loop needs a signal-check hook between every tool call, bounding cancel latency to the current tool call's duration. | `open-questions/02` B5. |
...
| [`04-durable-execution.md`](./open-questions/04-durable-execution.md) | Temporal vs Postgres job runner vs bare worker; step granularity; idempotency; cancellation | **Highest — least reversible.** Shapes how the graph itself is decomposed | 🔴 Not started. **Now carries two concrete inbound constraints from D33:** (1) the run controller must expose a signal-check hook at every tool-call boundary, not only at coarse checkpoints; (2) D33's Postgres-table + `LISTEN/NOTIFY` signal delivery is provisional on this session's orchestrator choice — confirm it still fits, or replace it with native signals (e.g. Temporal) and update D33 accordingly. |
