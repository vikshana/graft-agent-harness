...
| **Plugin Backend** | Go, Grafana plugin SDK | Proxy to API; Grafana Live `StreamHandler`; channel authz; forward `X-Grafana-Id`; **emit connect/teardown as the disconnect-clock signal** | Assert authorisation on the gateway's behalf (ADR-0023); stream SSE through the Go proxy (ADR-0031) | ADR-0001, ADR-0023, ADR-0031, ADR-0066 |
