...
Each becomes one or more ADRs in [`../adr/`](../adr/README.md). The four earlier
...
| DBOS × async LangGraph × `langchain-mcp-adapters` ergonomics — **blocking for C4 L3** | ADR-0037 |
| DBOS system-DB migrations vs `FORCE ROW LEVEL SECURITY` / PCI scope — **blocking for C4 L3** | ADR-0050 |
